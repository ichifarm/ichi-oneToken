...
contract("uniswapOracleTWAPCompare", accounts => {
    ...
    describe('Base Tests', async() => {
		...
		it("even the first call for consult should return price", async () => {
			let msg1 = "UniswapOracleTWAPCompare: INVALID_TOKEN";
			const amountOut = await uniswapOracleTWAPCompare.consult(memberTokenAddress, 10 ** 9);
			assert.notEqual(amountOut, 0, "amountOut should be not 0");
			await truffleAssert.reverts(uniswapOracleTWAPCompare.consult(badAddress, 1), msg1);
		});
		
		it("even the first call for amountRequired should return price", async () => {
			let msg1 = "UniswapOracleTWAPCompare: INVALID_TOKEN";
			const amountOut = await uniswapOracleTWAPCompare.amountRequired(memberTokenAddress, 10 ** 9);
			assert.notEqual(amountOut, 0, "amountOut should be not 0");
			await truffleAssert.reverts(uniswapOracleTWAPCompare.amountRequired(badAddress, 1), msg1);
		});
