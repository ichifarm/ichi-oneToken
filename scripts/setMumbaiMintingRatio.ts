...
async function main() {
  ...
  const ratio45   = "450000000000000000"; // 90%
  const ratio95   = "950000000000000000"; // 90%
  const ratio100 = "1000000000000000000"; // 90%
  const maxVol = "9999999999999999999999999999999999999999999999999"; // approximately unlimited

  const oneBTC = get('oneBTC');
  await execute('Incremental', {from: deployer, log: true}, 'setParam', oneBTC.address, ratio95, ratio100, 0, ratio100, maxVol);
